...
def find_file(minio_client, bucket: str):
    ...
    for obj in response["Contents"]:
        ...
        if "tournee_agents" in file_name:
...
def main() -> None:
    ...
    if key is None or file_bytes is None:
        print("Aucun fichier tournee_agent_raw transformé trouvé.")
        return
    ...
    print(f"Chargement de {file_name} vers raw.tournee_agent_raw")

    df = pd.read_csv(BytesIO(file_bytes))

    df.to_sql(
        name="tournee_agent_raw",
        con=engine,
        schema="raw",
        if_exists="append",
        index=False,
    ...
    print(f"Chargement terminé : raw.tournee_agent_raw ({len(df)} lignes)")
