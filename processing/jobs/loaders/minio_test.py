...
def load_ids(table, col):
    engine = get_engine()
    df = pd.read_sql(f"SELECT {col} FROM raw.{table}", engine)
...
def build_container_fill_profile_map(containers):
    """
    Profil stable par conteneur :
    - slow   : remplissage lent
    - normal : remplissage moyen
    - fast   : remplissage rapide
    """
    mapping = {}
...
def init_states(n):
    ...
    battery = np.random.uniform(70, 100, n)
...
def simulate_container_day(
    ...
    """
    - montée progressive dans le temps
    - chute uniquement aux timestamps de visite du jour
    - mesure injectée exactement à la visite
    """
    ...
    if fill_profile == "slow":
        fill_rate_per_sec = np.random.uniform(0.00008, 0.00018)
    elif fill_profile == "normal":
        fill_rate_per_sec = np.random.uniform(0.00018, 0.00040)
    else:
        fill_rate_per_sec = np.random.uniform(0.00040, 0.00080)
    ...
    for t in timestamps:
        ...
        # bruit léger, sans casser la logique
        fill_measured = fill + np.random.normal(0, 0.25)
        fill_measured = np.clip(fill_measured, 0, 120)
...
def simulate_day(
    ...
    for i, m in enumerate(counts):
        ...
        # visites du bon jour uniquement
        visit_seconds = visits.get((container_sk, date), [])

        # on injecte les visites dans les timestamps pour voir la chute exactement
        ...
        df = pd.DataFrame({
            ...
            "poids_estime_kg": kg,
            "is_overflow": (fill > 100).astype(int),
            "waste_type_sk": waste_type,
            "device_sk": np.random.choice(device_ids, len(all_timestamps))
        })
...
def simulate():
    ...
    device_ids = load_ids("dim_device_raw", "id")
