...
def find_file(minio_client, bucket: str):
    ...
    for obj in response["Contents"]:
        ...
        if "tournee_conteneurs" in file_name:
    ...
    selected_key = matching_files[0]

    file_obj = minio_client.get_object(Bucket=bucket, Key=selected_key)
    return selected_key, file_obj["Body"].read()
...
def main() -> None:
    ...
    if key is None or file_bytes is None:
        print("Aucun fichier tournee_conteneur_raw transformé trouvé.")
        return
    ...
    print(f"Chargement de {file_name} vers raw.tournee_conteneur_raw")

    df = pd.read_csv(BytesIO(file_bytes))

    df.to_sql(
        name="tournee_conteneur_raw",
        con=engine,
        schema="raw",
        if_exists="append",
        index=False,
    ...
    print(f"Chargement terminé : raw.tournee_conteneur_raw ({len(df)} lignes)")
