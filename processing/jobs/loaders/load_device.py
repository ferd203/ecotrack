...
def find_file(minio_client, bucket: str) -> tuple[str | None, bytes | None]:
    ...
    for obj in response["Contents"]:
        ...
        if "device_transformed" in file_name:
...
def main() -> None:
    ...
    if key is None or file_bytes is None:
        print("Aucun fichier dim_device_raw transformé trouvé.")
        return
    ...
    print(f"Chargement de {file_name} vers raw.dim_device_raw")
    ...
    print(f"Chargement terminé : raw.dim_device_raw ({len(df)} lignes)")
