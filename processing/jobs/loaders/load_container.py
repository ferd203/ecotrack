...
def find_file(minio_client, bucket: str) -> tuple[str | None, bytes | None]:
    ...
    for obj in response["Contents"]:
        ...
        if "container_transformed" in file_name:
...
def main() -> None:
    ...
    if key is None or file_bytes is None:
        print("Aucun fichier dim_container_raw transformé trouvé.")
        return
    ...
    print(f"Chargement de {file_name} vers raw.dim_container_raw")

    df = pd.read_csv(BytesIO(file_bytes))

    df.to_sql(
        name="dim_container_raw",
        con=engine,
        schema="raw",
        if_exists="append",
        index=False,
    ...
    print(f"Chargement terminé : raw.dim_container_raw ({len(df)} lignes)")
