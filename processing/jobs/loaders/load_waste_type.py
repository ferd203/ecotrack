...
def find_waste_type_file(minio_client, bucket: str) -> tuple[str | None, bytes | None]:
    ...
    for obj in response["Contents"]:
        ...
        if "waste_type_transformed" in file_name:
...
def main() -> None:
    ...
    key, file_bytes = find_waste_type_file(minio_client, bucket)

    if key is None or file_bytes is None:
        print("Aucun fichier DIM_WASTE_TYPE transformé trouvé.")
        return
    ...
    print(f"Chargement de {file_name} vers raw.dim_waste_type_raw")

    df = pd.read_csv(BytesIO(file_bytes))

    df.to_sql(
        name="dim_waste_type_raw",
        con=engine,
        schema="raw",
        if_exists="append",
        index=False,
    ...
    print(f"Chargement terminé : raw.dim_waste_type_raw ({len(df)} lignes)")
